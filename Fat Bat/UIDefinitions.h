//
//  UIDefinitions.h
//  Fat Bat
//
//  Created by Wil Pirino on 12/16/15.
//  Copyright © 2015 Wil Pirino. All rights reserved.
//

#ifndef UIDefinitions_h
#define UIDefinitions_h


#define LEVELS_FILE_NAME @"levels"


#define UPDATE_TIME_INTERVAL 0.2


#define MAX_FLY_ANGLE M_PI/8.0 

#define BAT_X_OFFSET 32.0
#define BAT_Y_OFFSET 64.0


#define CAVE_CEILING_HEIGHT 16.0
#define CAVE_FLOOR_HEIGHT 16.0


#define FINISH_LINE_WIDTH 59.0
#define FINISH_LINE_NUM_COLUMNS 4


#define BORDER_WIDTH 2.0

#define FONT_NAME @"HelveticaNeue-Bold"
#define FONT_SIZE 20.0
#define TITLE_FONT_SIZE 64.0
#define DETAIL_FONT_SIZE 12.0


#define BUTTON_CORNER_RADIUS 8.0

#define GAME_BUTTON_OFFSET 8.0
#define GAME_BUTTON_WIDTH 80.0
#define GAME_BUTTON_HEIGHT 40.0

#define PLAY_BUTTON_WIDTH 160.0
#define PLAY_BUTTON_HEIGHT 80.0

#define TITLE_LABEL_WIDTH 320.0
#define TITLE_LABEL_HEIGHT 80.0


#define OUTER_CAVE_RED 255.0/255.0
#define OUTER_CAVE_GREEN 178.0/255.0
#define OUTER_CAVE_BLUE 102.0/255.0

#define INNER_CAVE_RED 153.0/255.0
#define INNER_CAVE_GREEN 76.0/255.0
#define INNER_CAVE_BLUE 0.0/255.0


#define OPAQUE_RED 0.35
#define OPAQUE_GREEN 0.35
#define OPAQUE_BLUE 0.35
#define OPAQUE_ALPHA 0.35


#endif /* UIDefinitions_h */
